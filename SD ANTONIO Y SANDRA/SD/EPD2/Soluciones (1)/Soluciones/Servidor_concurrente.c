...
int main(void){
    ...
    while(1){
        ...
        // Cada vez que llegue una petición, lanzamos un proceso para atenderla.

        if ((pid=fork())==-1) perror("ERROR AL CONSTRUIR EL PROCESO HIJO");
            ...
            if (pid != 0) {
                ...
                printf("[SERVIDOR, PID = %d] Datos recibidos del cliente: %s\n",pid,datosRecibidos);
                ...
                printf("[SERVIDOR, PID = %d]  Voy a enviar el siguiente mensaje: %s\n",pid,resultadoOperacion);
                ...
                // **** Eliminamos el proceso hijo ya que ha terminado de atender al cliente (señal SIGTERM).
                kill(pid,SIGTERM);
            }
